...
1.  **Global Authority (magnitude anchor):** The Global Model is the magnitude anchor for `total_kwh` — corrected only by external factors (solar gain, aux reduction), never replaced by the per-unit base sum. This is the canonical statement of **Component Boundary Invariant #8** in `CLAUDE.md`; see there for the precise formula, the two scale invariants (anchor-scale aux clamp; solar re-saturated against the scaled allocation), and the cold-start fallback. The per-unit sum lacks the global U-curve's self-correction above the balance point — anchoring on it regressed every install's warm-day prediction (#992 c2, restored in #1035).
2.  **Honest reporting, NOT proportional scaling:** Per-unit reductions are reported *as learned* — they are NOT scaled to force their sum to equal the Global target. (This supersedes the earlier "Proportional Scaling" design; scaling masked Track-A-vs-Track-B disagreement instead of surfacing it.) The disagreement is exposed explicitly: `orphaned_aux_savings` (Track A aux > per-entity sum) and `unspecified_kwh` (global anchor vs. per-entity net attribution delta).
...
### Thermal Hysteresis Across Season Transitions

The model treats identical outdoor temperatures as thermodynamically equivalent, regardless of the thermal history that preceded them. In practice, a house at +2 °C after a week below −10 °C behaves very differently from a house at +2 °C after cooling down from a mild period. This is **not confined to deep cold spells** — the same mechanism operates at every season transition, in both directions. The building's slow thermal mass carries a hidden state that lags outdoor air by days to weeks, so the same `(temp, wind)` bucket represents a different physical heat-loss regime in autumn (mass still warm) than in spring (mass still cold). A deep cold spell is simply the most acute instance, not the boundary of the phenomenon.
...
**Observed symptom:** The bias is signed by the direction of thermal travel and recurs at every transition. Warming toward a milder regime (spring, or recovery after any multi-day cold snap) the model *underestimates* demand; cooling from a milder baseline (autumn, or the onset of a cold snap) it *overestimates* demand at the same outdoor temperatures. Each transition then produces a fresh drift as the affected buckets re-converge to the new regime. The asymmetry is most pronounced near +1 °C to +3 °C.
